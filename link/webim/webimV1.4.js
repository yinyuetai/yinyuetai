...
var webim = { // namespace object webim
	 ...
	 *     identifier	- String, 用户帐号，必填
         *     identifierNick	- String, 用户昵称，选填
	 *     userSig		- String, 鉴权Token，必填
	 *   }
	 ...
	 *   (无)
	 */
	syncMsgs: function(options,cbOk, cbErr) {},
 ...
 */
(function(webim) {
	var SRV_HOST = "https://webim.tim.qq.com";
        var PIC_SRV_HOST = "https://pic.tim.qq.com";
        
        var browserInfo={};//浏览器版本信息
        
        var SRV_NAME_OPEN_IM='openim';//私聊（拉取未读c2c消息，长轮询，c2c消息已读上报等）服务名
        var SRV_NAME_GROUP='group_open_http_svc';//群组管理（拉取群消息，创建群，群成员管理，群消息已读上报等）服务名
        var SRV_NAME_FRIEND='sns';//关系链管理（好友管理，黑名单管理等）服务名
        var SRV_NAME_PROFILE='profile';//资料管理（查询，设置个人资料等）服务名
        var SRV_NAME_PIC='openpic';//图片（上传图片）服务名
        var SRV_NAME_VER={//不同服务对应的版本号
            'openim':'v4',
        ...
        var SDK_VERSION='1.4';//web im sdk版本号
        var SDK_APPID='537046246 ';//web im sdk APPID
        var MSG_TYPE_C2C='C2C';//私聊类型
        var MSG_TYPE_GROUP='GROUP';//群聊类型
        var ACTION_STATUS_OK='OK';//后台接口返回成功
        var ACTION_STATUS_FAIL='FAIL';//后台接口返回失败
        var ERROR_CODE_CUSTOM=99999;//自定义后台接口返回错误码
        
        //消息类型
        var MSG_ELEMENT_TYPE_TEXT='TIMTextElem';//文本
        var MSG_ELEMENT_TYPE_FACE='TIMFaceElem';//表情
        var MSG_ELEMENT_TYPE_IMAGE='TIMImageElem';//图片
        
        //只支持显示
        var MSG_ELEMENT_TYPE_SOUND='TIMSoundElem';//语音
        var MSG_ELEMENT_TYPE_FILE='TIMFileElem';//文件
        
        //以下类型，web端暂不支持
        var MSG_ELEMENT_TYPE_LOCATION='TIMLocationElem';//地理位置
        var MSG_ELEMENT_TYPE_CUSTOM='TIMCustomElem';//自定义
        
        //新增群提示消息类型
        var MSG_ELEMENT_TYPE_GROUP_TIP="TIMGroupTipElem";//群提示消息
        ...
        //下载文件业务ID
        var DOWNLOAD_FILE_BUSSINESS_ID="10001";
        //下载文件authkey
        var DOWNLOAD_FILE_AUTH_KEY="617574686b6579";
        //下载文件服务器IP
        var DOWNLOAD_FILE_SERVER_IP="182.140.186.147";
        //下载文件类型
	...
	var getApiUrl = function(srvName,cmd) {
                ...
                if (srvName==SRV_NAME_PIC){
                    srvHost=PIC_SRV_HOST;
                }
                var url=srvHost+'/'+SRV_NAME_VER[srvName]+'/'+srvName+'/'+cmd+'?websdkappid='+SDK_APPID+"&v="+SDK_VERSION;
                url+='&sdkappid='+ctx.sdkAppID+'&identifier='+encodeURIComponent(ctx.identifier)+'&appidat3rd='+ctx.appIDAt3rd+'&accounttype='+ctx.accountType+'&usersig='+ctx.userSig+'&apn='+ctx.apn+'&contenttype='+ctx.contentType+'&reqtime='+unixtime();

		return url;
	};
        
        var initLoginInfo=function(loginInfo){
	...
	// REST API calls
        //上线
	var proto_login = function( loginInfo,cbOk, cbErr) {
		initLoginInfo(loginInfo);
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"login", {"State":"Online"}, 
                            function(loginResp){
                                var tag_list = [
                                    "Tag_Profile_IM_Nick"
	...
	var proto_offline = function( cbOk, cbErr) {
		...
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"logout", {}, 
                        function(resp){
                            //遍历xmlHttpObjMap{}
	...
	var proto_sendMsg = function(msg, cbOk, cbErr) {
		...
		for (var i in msg.elems) {
			...
			msgInfo.MsgBody.push({'MsgType':msgType,'MsgContent':msgContent});
                ...
                if(msg.sess.type()==MSG_TYPE_C2C){//私聊
                    ConnManager.apiCall(SRV_NAME_OPEN_IM,"sendmsg", msgInfo, cbOk, cbErr);
                }else if(msg.sess.type()==MSG_TYPE_GROUP){//群聊
                    ConnManager.apiCall(SRV_NAME_GROUP,"send_group_msg", msgInfo, cbOk, cbErr);
                }
		...
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"longpolling", options, cbOk, cbErr);
	};
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"getmsg", {'Cookie':cookie,'SyncFlag':syncFlag}, 
                                        function(resp){
		...
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"msgreaded", {C2CMsgReaded:{'Cookie':cookie,'C2CMsgReadedItem':tmpC2CMsgReadedItem}}, cbOk, cbErr);
	};
		...
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"deletemsg",options , 
                                   cbOk, cbErr);
	};
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"getroammsg", options, 
                                        function(resp){
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"create_group", opt, 
                                   cbOk, cbErr);
	};
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"modify_group_base_info", options, 
                                   cbOk, cbErr);
	};
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"apply_join_group", {
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"handle_apply_join_group", {
                                                'GroupId':options.GroupId,
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"quit_group", {
                                                'GroupId':options.GroupId
                                              }, 
                                   cbOk, cbErr);
	};
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"get_group_public_info", {
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"get_group_info", opt, 
                                   cbOk, cbErr);
	};
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"get_group_member_info", {
                                                'GroupId':options.GroupId,
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"add_group_member", {
                                                'GroupId':options.GroupId,
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"modify_group_member_info", opt, 
                                   cbOk, cbErr);
	};
        //删除群组成员
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"delete_group_member", {
                                                'GroupId':options.GroupId,
                                                'Silence':options.Silence,
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"destroy_group", {
                                                'GroupId':options.GroupId
                                              }, 
                                   cbOk, cbErr);
	};
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"get_joined_group_list", {
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"get_role_in_group", {
                                                'GroupId':options.GroupId,
		...
		ConnManager.apiCall(SRV_NAME_GROUP,"forbid_send_msg", {
                                                'GroupId':options.GroupId,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_GROUP,"send_group_system_notification", options, 
                                   cbOk, cbErr);
	};
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_GROUP,"group_msg_get", {
                                              "GroupId":options.GroupId,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_GROUP,"msg_read_report", {
                                              'GroupId':options.GroupId,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"friend_delete", {
                                              'From_Account':options.From_Account,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"pendency_get", {
                                              "From_Account":options.From_Account,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"pendency_delete", {
                                              "From_Account":options.From_Account,
                                              "PendencyType":options.PendencyType,
                                              "To_Account":options.To_Account
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"friend_response", {
                                              'From_Account':options.From_Account,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"friend_get_all", {
                                              'From_Account':options.From_Account,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_PROFILE,"portrait_get", {
                                              'From_Account':options.From_Account,
                                              'To_Account':options.To_Account,
                                              'LastStandardSequence':options.LastStandardSequence,
                                              'TagList':options.TagList
		...
		ConnManager.apiCall(SRV_NAME_PROFILE,"portrait_set", 
                                          {
                                              'From_Account':options.From_Account,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"black_list_add", {
                                              'From_Account':options.From_Account,
                                              'To_Account':options.To_Account
                                          }, 
                                    function(resp){
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"black_list_delete", {
                                              'From_Account':options.From_Account,
                                              'To_Account':options.To_Account
                                          }, 
                                    function(resp){
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_FRIEND,"black_list_get", {
                                              'From_Account':options.From_Account,
                                              'StartIndex':options.StartIndex,
                                              'MaxLimited':options.MaxLimited,
                                              'LastSequence':options.LastSequence
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_PIC,"pic_up", {
                                              'App_Version':SDK_APPID,
                                              'From_Account':options.From_Account,
                                              'To_Account':options.To_Account,
                                              'Seq':options.Seq,
                                              'Timestamp':options.Timestamp,
		...
		if (!checkLogin()) return;
		ConnManager.apiCall(SRV_NAME_OPEN_IM,"authkey", {}, cbOk, cbErr);
	};
        //end
        var lowerBR = false;
	...
	var ConnManager = lowerBR == false 
		...
		this.apiCall = function(type,cmd, data, cbOk, cbErr) {
			...
			ajaxRequestJson("POST", url, data, function(resp) {
                                //成功
				if (resp.ActionStatus == ACTION_STATUS_OK) {
	...
	// class Msg
	var Msg = function(sess, isSend, seq, random,time,fromAccount,type,fromAccountNick) {
		this.sess = sess;
	...
	Msg.prototype.addText = function(text) {
		this.addElem(new webim.Msg.Elem(MSG_ELEMENT_TYPE_TEXT, text));
	};
        //表情
	Msg.prototype.addFace = function(face) {
		this.addElem(new webim.Msg.Elem(MSG_ELEMENT_TYPE_FACE, face));
	};
        //图片
	Msg.prototype.addImage = function(image) {
		this.addElem(new webim.Msg.Elem(MSG_ELEMENT_TYPE_IMAGE, image));
	};
        //自定义
	Msg.prototype.addCustom = function(custom) {
		this.addElem(new webim.Msg.Elem(MSG_ELEMENT_TYPE_CUSTOM, custom));
	};
		...
		this.setAutoRead = function(selSess, isOn, isResetAll) {
			...
			if (selSess) {
				...
				if (isOn) {//是否调用已读上报接口
                                    ...
                                    if(selSess._impl.type==MSG_TYPE_C2C){//私聊消息已读上报
                                        var tmpC2CMsgReadedItem=[];
                                    ...
                                    }else if(selSess._impl.type==MSG_TYPE_GROUP){//群聊消息已读上报
                                        var tmpOpt={
	...
	var MsgManager = new function() {
		...
		var onGroupInfoChangeCallback = null;//群资料变化回调
                ...
                var initMyGroupMaxSeqs=function(cbOk,cbErr){
                    ...
                    proto_getJoinedGroupListHigh(opts,function(resp){
                            ...
                            log.warn("initMyGroupMaxSeqs success");
                        },
                        function(err) {
                ...
                var addGroupMsgList=function(msgs,new_group_msgs){
                    for(var p in msgs){
                            ...
                            if(newGroupMsg.From_Account){
                                var msg=handlerGroupMsg(newGroupMsg.ToGroupId,newGroupMsg,false);
                                    if(msg){
                ...
                var handlerGroupTips=function(groupTips){
                                    ...
                                    for(var o in groupTips){
                                        ...
                                        //添加event字段
                                        groupTip.Event=LONG_POLLINNG_MSG_TYPE.GROUP_TIP;
                                        //更新群消息通知seq
                                        if(groupTip.NoticeSeq>noticeSeq){
                                            noticeSeq=groupTip.NoticeSeq;
                                        }
                                        var msg=handlerGroupMsg(groupTip.ToGroupId,groupTip,false);
                                        if(msg){
                                            new_group_msgs.push(msg);
                                        }
                                    }
                ...
                var longPolling=function(cbOk,cbErr){
                    ...
                    proto_longPolling(opts,function(resp) {
                        for(var i in resp.EventArray){
                            ...
                            switch(e.Event){
                                case LONG_POLLINNG_MSG_TYPE.C2C://c2c消息通知
                                    //更新C2C消息通知seq
		...
		this.syncMsgs = function(cbOk, cbErr) {
			...
			proto_getMsgs(MsgStore.cookie, MsgStore.syncFlag,function(resp) {
				...
				for (var i in msgInfos) {
					...
					var sess = MsgStore.sessByTypeId(MSG_TYPE_C2C, id);
					if (!sess) {	
						sess = new Session(MSG_TYPE_C2C, id, id, headUrl, 0,0);
					}
		...
		this.getC2CHistoryMsgs = function(options,cbOk, cbErr) {
                        ...
                        if(options.MaxCnt>100){
                            alert('一次拉取的条数不能超过100条');
                            return;
                        }
                        var msgKey=C2CHitoryMsgKeyMap[options.Peer_Account];
		...
		this.syncGroupMsgs = function(options,cbOk, cbErr) {
			...
			proto_getGroupMsgs(opts,function(resp) {
				...
				for (var i=msgInfos.length-1;i>=0;i--) {
					...
					var msg=handlerGroupMsg(group_id,msgInfo,true);
                                        if(msg){
                                            notifyInfo.push(msg);
                                        }
				} // for loop
                ...
                var handlerGroupMsg=function (group_id,msgInfo,isSyncGroupMsgs){
					...
					var sess = MsgStore.sessByTypeId(MSG_TYPE_GROUP, group_id);
					if (!sess) {
						sess = new Session(MSG_TYPE_GROUP, group_id, group_name, headUrl, 0,0);
					}
                                        var subType=GROUP_MSG_SUB_TYPE.COMMON;//消息类型
                                        //群提示消息,重新封装下
                                        if(LONG_POLLINNG_MSG_TYPE.GROUP_TIP==msgInfo.Event){
                                            subType=GROUP_MSG_SUB_TYPE.TIP;
                                            var groupTip=msgInfo.MsgBody;
                                            msgInfo.MsgBody=[];
                                            msgInfo.MsgBody.push({
                                                    "MsgType":MSG_ELEMENT_TYPE_GROUP_TIP,
                                                    "MsgContent":groupTip
                                                }
                                            );
					...
					for (var mi in msgInfo.MsgBody) {
                                                ...
                                                switch(msgType){
                                                    ...
                                                    case MSG_ELEMENT_TYPE_FILE+" ":
                                                        ...
                                                        break;
                                                    case MSG_ELEMENT_TYPE_GROUP_TIP:
                ...
                //初始化
		this.init = function(onMsgNotify,onGroupInfoChangeNotify,groupSystemNotifys) {
			onMsgCallback = onMsgNotify;
                        ...
                        log.info('browserInfo: type='+browserInfo.type+', ver='+browserInfo.ver);
                        if(browserInfo.type=='ie' && parseInt(browserInfo.ver)<8){
		...
		this.sendMsg = function(msg, cbOk, cbErr) {
			proto_sendMsg(msg, function(resp) {
                ...
                var getSoundDownUrl=function(uuid,senderId){
                    var soundUrl=null;
                    if(authkey && ipList[0]){
                        soundUrl="http://"+ipList[0]+"/asn.com/stddownload_common_file?authkey="+authkey+"&bid="+DOWNLOAD_FILE_BUSSINESS_ID+"&subbid="+ctx.appIDAt3rd+"&fileid="+uuid+"&filetype="+DOWNLOAD_FILE_TYPE.SOUND+"&openid="+senderId+"&ver=0";
                    }else{
                ...
                var getFileDownUrl=function(uuid,senderId,fileName){
                    var fileUrl=null;
                    if(authkey && ipList[0]){
                        fileUrl="http://"+ipList[0]+"/asn.com/stddownload_common_file?authkey="+authkey+"&bid="+DOWNLOAD_FILE_BUSSINESS_ID+"&subbid="+ctx.appIDAt3rd+"&fileid="+uuid+"&filetype="+DOWNLOAD_FILE_TYPE.FILE+"&openid="+senderId+"&ver=0&filename="+encodeURIComponent(fileName);
                    }else{
	...
	webim.init = function(loginInfo, listeners, options) {
		...
		MsgManager.init(listeners.onMsgNotify,listeners.onGroupInfoChangeNotify,listeners.groupSystemNotifys);
	};
	...
	webim.MsgStore = {
        ...
        //
        //用户上线
        webim.login = function(loginInfo,cbOk, cbErr) {return proto_login(loginInfo,cbOk, cbErr);};
        //用户下线
